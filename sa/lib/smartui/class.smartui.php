...
class SmartUI extends SmartUtil {
     ...
     * print_alert
     * --------------------------------------------------
     * @param string $message message to display in alert box
     * @param string $type the type of alert box to display
     * @param mixed $options array of options or class string
     * @param boolean $return true to return, false to print
     * --------------------------------------------------
     **/
    public static function print_alert($message, $type = 'info', $options = array(), $return = false)
        {
            // The defaults for an alert box
            $options_map = array(
                'closebutton' => true,
                'block'     => false,
                'container' => 'div',
            ...
            // Icon to use on different alert types
            $icon_map = array(
                'info'      => 'fa-info-circle',
            ...
            // If $options is a string, treat it as $options['class'] 
            $new_options_map = parent::set_array_prop_def($options_map, $options, 'class');

            // Create class string for alert box
            $classes = array();
            $classes[] = 'alert';
            $classes[] = 'alert-'.$type;
            if ($new_options_map['fade_in']) $classes[] = 'fade in';
            if ($new_options_map['block']) $classes[] = 'alert-block';
            if( $new_options_map['class']) $classes[] = implode(' ', $new_options_map['class']);
            
            // Create alert box HTML
            $closebutton_html = $new_options_map['closebutton'] ? '<button class="close" data-dismiss="alert">&times;</button>' : '';
            $icon_html = $new_options_map['icon'] ? '<i class="fa-fw fa '.(isset($new_options_map['icon']) ? 'fa-'.$new_options_map['icon'] : $icon_map[$type]).'"></i>' : '';
        	$result = '<'.$new_options_map['container'].' class="'.implode(' ', $classes).'">'.$closebutton_html.$icon_html.$message.'</'.$new_options_map['container'].'>';
        	
            // Return or print the html
            if ($return) 
                return $result;
        	echo $result;
        }
